...
{
	public partial class NetworkedItemContainer : BaseNetworkable, IItemContainer, INetworkSerializer
	{
		...
		~NetworkedItemContainer()
		{
			IItemContainer.All.Remove( this );
		}

		public NetworkedItemContainer() { }
		public NetworkedItemContainer( string uuid, Entity entity, int slots )
		{
			UUID = uuid;
			Entity = entity;
			Slots = slots;
		...
		public NetworkedItemContainer( Entity entity, ItemContainerPersistentData iData ) : this( iData.UUID, entity, iData.Slots )
		{
			Items = iData.Items;
			IsPersistent = true;
		...
		{
			(bool success, int leftover) = Realm.Assert( RealmType.Server, InventoryHelper.TryGiveItem( this, item ) );
			if ( success )
			{
		...
		{
			(bool success, int leftover) = Realm.Assert( RealmType.Server, InventoryHelper.TryTakeItem( this, item ) );
			if ( success )
			{
		...
		{
			changes.Add( (EventType.Added, id, amountGained) );
			WriteNetworkData();
		}

		protected virtual void OnItemsRemoved( uint id, int amountRemoved )
		{
			changes.Add( (EventType.Removed, id, amountRemoved) );
			WriteNetworkData();
		}
